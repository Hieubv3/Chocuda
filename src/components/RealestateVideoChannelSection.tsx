...
export const RealestateVideoChannelSection: React.FC<RealestateVideoChannelSectionProps> = ({
  ...
  const handleAddVideo = (e: React.FormEvent) => {
    ...
    if (!ytId) {
      alert(' Đường dẫn YouTube không hợp lệ. Vui lòng dán đường dẫn dạng:\n• https://www.youtube.com/watch?v=...\n• https://youtu.be/...\n• https://www.youtube.com/shorts/...');
      return;
    }
  ...
  return (
    <div className="space-y-6 bg-ink-900 text-white rounded-3xl p-6 sm:p-8 border border-ink-800 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-ink-800 pb-6 relative z-10">
        <div>
          <span className="text-xs font-black uppercase text-brand-400 tracking-widest flex items-center gap-1.5">
          ...
          <p className="text-xs sm:text-sm text-ink-400 mt-1">
            Hiển thị trực tiếp các video chính chủ đăng tải tại kênh YouTube & TikTok <b className="text-brand-300">@chocudan24h</b>.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0">
          {isAdmin && (
            <button
              ...
              className="px-4 py-3 bg-brand-500 hover:bg-brand-600 active:bg-brand-700 text-ink-950 font-black text-xs sm:text-sm rounded-xl transition shadow-lg flex items-center gap-1.5 transform hover:-translate-y-0.5"
            >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-4 py-3 bg-ink-800 hover:bg-ink-700 text-brand-400 font-extrabold text-xs sm:text-sm rounded-xl transition border border-ink-700 flex items-center gap-2 transform hover:-translate-y-0.5"
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition flex items-center gap-2 ${
              activeTab === 'youtube'
                ...
                : 'bg-ink-800/80 text-ink-400 hover:bg-ink-800 hover:text-white'
            }`}
          >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-ink-900 border border-ink-700 rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl relative text-xs">
            <div className="flex items-center justify-between border-b border-ink-800 pb-3">
              ...
              <button
                ...
                className="text-ink-400 hover:text-white font-bold text-lg"
              >
            ...
            <form onSubmit={handleAddVideo} className="space-y-3">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-ink-950 border border-ink-700 rounded-xl px-3 py-2.5 text-white placeholder-ink-500 focus:outline-none focus:ring-2 focus:ring-brand-500 text-xs"
                  />
                  <LinkIcon className="w-4 h-4 text-ink-500 absolute right-3 top-3 pointer-events-none" />
                </div>
              </div>

              <div>
                <label className="block text-ink-300 font-bold mb-1">Tiêu đề Video hiển thị:</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-ink-300 font-bold mb-1">Phân loại BĐS / Dự án:</label>
                <select
                  value={newVideoCategory}
                  onChange={(e) => setNewVideoCategory(e.target.value)}
                  className="w-full bg-ink-950 border border-ink-700 rounded-xl px-3 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-brand-500 text-xs"
                >
              ...
              <div className="p-3 bg-brand-500/10 border border-brand-500/30 rounded-xl text-[11px] text-brand-300 leading-relaxed">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="px-4 py-2 bg-ink-800 hover:bg-ink-700 text-ink-300 font-bold rounded-xl"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-brand-500 hover:bg-brand-600 text-ink-950 font-black rounded-xl shadow-lg"
                >
                   Cập Nhật Video Ngay
                </button>
              </div>
      ...
      {activeTab === 'youtube' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
          ...
          <div className="lg:col-span-8 space-y-3">
            {currentVideo ? (
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-ink-950 border border-ink-800 shadow-2xl">
                <iframe
              ...
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-ink-950 border border-ink-800 flex items-center justify-center">
                <p className="text-xs text-ink-400">Chưa có video nào. Bấm "Thêm Video Kênh Của Tôi" để dán link YouTube!</p>
              </div>
            )}

            {/* Currently Playing Info */}
            {currentVideo && (
              <div className="p-4 bg-ink-800/60 rounded-2xl border border-ink-800 flex items-start justify-between gap-4">
                <div>
                  <span className="text-[10px] font-extrabold uppercase bg-brand-500/20 text-brand-400 px-2.5 py-1 rounded-full border border-brand-500/30">
                  ...
                  <div className="flex items-center gap-3 text-xs text-ink-400 mt-1">
                    ...
                    <span>•</span>
                    <span className="text-brand-400 font-semibold"> Video chính chủ</span>
                  </div>
                </div>
          ...
          <div className="lg:col-span-4 space-y-3 max-h-[480px] overflow-y-auto pr-1">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-black uppercase text-ink-400 tracking-wider">
            ...
            {videoList.length === 0 ? (
              <div className="p-4 text-center border border-dashed border-ink-700 rounded-2xl text-ink-500 text-xs">
                  ...
                  <div
                    ...
                    className={`p-3 rounded-2xl border transition cursor-pointer flex gap-3 items-center group relative ${
                      isPlaying
                        ...
                        : 'bg-ink-800/50 border-ink-800 hover:bg-ink-800 text-ink-300'
                    }`}
                  >
                    <div className="relative w-28 aspect-video rounded-xl overflow-hidden bg-ink-950 shrink-0">
                      <img loading="lazy"
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <Play className={`w-6 h-6 ${isPlaying ? 'text-brand-400 fill-brand-400' : 'text-white'}`} />
                      </div>
                    </div>
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="absolute right-2 top-2 text-ink-500 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition p-1"
                      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
          {[
            ...
            <div
              key={i}
              className="p-4 bg-ink-800/80 border border-ink-700/80 rounded-2xl space-y-3 hover:border-brand-500/50 transition group"
            >
              <div className="aspect-[9/14] bg-ink-950 rounded-xl overflow-hidden relative border border-ink-700 flex items-center justify-center">
                <img loading="lazy"
                  src={''}
                  alt={item.title}
                  className="w-full h-full object-cover group-hover:scale-105 transition duration-300"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-ink-950 via-black/20 to-transparent flex flex-col justify-between p-3">
                  <div className="flex justify-between items-center">
                    <span className="bg-brand-500 text-ink-950 text-[10px] font-black px-2 py-0.5 rounded-full">
                      TikTok @chocudan24h
                    </span>
                    <span className="text-[10px] text-white/90 font-bold"> {item.likes}</span>
                  </div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="w-10 h-10 bg-brand-500 text-ink-950 rounded-full flex items-center justify-center mx-auto shadow-lg group-hover:scale-110 transition"
                  >
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-brand-300 font-semibold">{item.user}</span>
                  </div>
                </div>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-2 bg-ink-700 hover:bg-ink-600 text-brand-300 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition"
              >
      ...
      {/* Bottom Info Banner */}
      <div className="p-4 bg-ink-800/40 rounded-2xl border border-ink-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-ink-400">
        <div className="flex items-center gap-2">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-brand-400 font-bold hover:underline shrink-0"
        >
