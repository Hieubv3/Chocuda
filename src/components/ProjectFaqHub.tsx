...
export const ProjectFaqHub: React.FC<ProjectFaqHubProps> = ({
  ...
  return (
    <div className={`bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl ${className}`}>
      {/* Dynamic JSON-LD Schema Injector */}
      <SeoJsonLd type="faq" faqItems={filteredFaqs} />

      {/* Header & Update Schedule Indicator */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-ink-100 dark:border-ink-800 pb-5">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-black text-ink-900 dark:text-white">
            Cẩm Nang Giải Đáp Pháp Lý, Quy Hoạch & Đầu Tư BĐS Vinhomes
          </h2>
        </div>

        {/* Weekly Auto-Update Badge */}
        <div className="flex items-center gap-2 bg-ink-50 dark:bg-ink-950 px-4 py-2.5 rounded-2xl border border-ink-200 dark:border-ink-800 text-xs font-bold shrink-0">
          <RefreshCw className="w-4 h-4 text-brand-500 animate-spin" style={{ animationDuration: '6s' }} />
          <div>
            <p className="text-ink-900 dark:text-white font-extrabold flex items-center gap-1">
              <span>Cập Nhật Tự Động: 1 Tuần / Lần</span>
              <span className="w-2 h-2 rounded-full bg-brand-500 inline-block animate-ping"></span>
            </p>
            <p className="text-[10px] text-ink-400 font-normal">
              Lịch cập nhật tiếp theo: <strong className="text-brand-500">Thứ 2 Tuần Tới</strong>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="relative w-full md:flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-ink-50 dark:bg-ink-950 border border-ink-200 dark:border-ink-800 rounded-2xl text-xs font-bold text-ink-900 dark:text-white focus:ring-2 focus:ring-brand-500"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 text-xs font-extrabold">
          <button
            ...
            className={`px-3 py-2.5 rounded-xl transition whitespace-nowrap ${
              activeCategory === 'all'
                ? 'bg-brand-500 text-ink-950 shadow-md font-black'
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-4 sm:p-5 text-left flex items-start justify-between gap-3 font-extrabold text-xs sm:text-sm text-ink-900 dark:text-white hover:text-brand-500 transition"
                >
                  <span className="flex items-start gap-2.5">
                    <span className="px-2 py-0.5 bg-brand-500/20 text-brand-500 rounded font-black text-[10px] uppercase shrink-0 mt-0.5">
                ...
                {isOpen && (
                  <div className="px-4 sm:px-5 pb-5 pt-1 text-xs text-ink-600 dark:text-ink-300 space-y-3 border-t border-ink-100 dark:border-ink-800/60 mt-1">
                    <p className="leading-relaxed text-ink-700 dark:text-ink-200 font-medium">
                      {item.answer}
                    </p>

                    {/* SEO Keyword Tags */}
                    <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-ink-200/50 dark:border-ink-800/40 text-[10px]">
                      <span className="text-ink-400 font-bold">Từ khóa SEO AI Google:</span>
                      {item.keywords.map((kw, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-0.5 bg-ink-200/60 dark:bg-ink-800 text-ink-600 dark:text-ink-400 rounded-md font-semibold"
                        >
                          #{kw}
                        </span>
                      ))}
