...
import { Building2, Phone, Heart, Scale, User, ShieldCheck, Globe, Menu, X, PlusCircle, Sparkles, Sun, Moon, Zap, KeyRound, Share2, ChevronDown, Home, Store, Wrench, Smartphone, Download } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 shadow-xs transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-[56px] py-1.5 gap-1">
          ...
          <nav className="hidden lg:flex items-center space-x-0.5 xl:space-x-1">
            ...
            >
              ...
              {saleHover && (
                <div className="absolute top-full left-0 mt-1 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-3 z-50 space-y-2 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="space-y-1">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl hover:bg-emerald-50 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-bold text-xs flex items-center justify-between group/sub"
                    >
                    ...
                    <div className="pl-8 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                      <button onClick={() => handleNavFilter('sale', 'cao-tang', 'studio')} className="block hover:text-emerald-600">Studio & 1PN (Đầu tư tốt)</button>
                      <button onClick={() => handleNavFilter('sale', 'cao-tang', '2pn')} className="block hover:text-emerald-600">Căn hộ 2PN (Gia đình trẻ)</button>
                      <button onClick={() => handleNavFilter('sale', 'cao-tang', '3pn')} className="block hover:text-emerald-600">Căn hộ 3PN+ (Không gian rộng)</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="pl-8 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                      <button onClick={() => handleNavFilter('sale', 'thap-tang', 'shophouse')} className="block hover:text-emerald-600">Shophouse Thương Mại</button>
                      <button onClick={() => handleNavFilter('sale', 'thap-tang', 'lien-ke')} className="block hover:text-emerald-600">Nhà Liền Kề Phố</button>
                      <button onClick={() => handleNavFilter('sale', 'thap-tang', 'biet-thu-song-lap')} className="block hover:text-emerald-600">Biệt Thự Song Lập & Đơn Lập</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {rentHover && (
                <div className="absolute top-full left-0 mt-1 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-3 z-50 space-y-2 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="pl-8 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                      <button onClick={() => handleNavFilter('rent', 'cao-tang', 'studio')} className="block hover:text-emerald-600">Cho thuê Studio & 1PN</button>
                      <button onClick={() => handleNavFilter('rent', 'cao-tang', '2pn')} className="block hover:text-emerald-600">Cho thuê Căn Hộ 2PN Full Đồ</button>
                      <button onClick={() => handleNavFilter('rent', 'cao-tang', '3pn')} className="block hover:text-emerald-600">Cho thuê Căn Hộ 3PN VIP</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="pl-8 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                      <button onClick={() => handleNavFilter('rent', 'thue-tang', 'thue-tang')} className="block hover:text-emerald-600">Thuê Tầng / Mặt Bằng Kinh Doanh</button>
                      <button onClick={() => handleNavFilter('rent', 'thap-tang', 'shophouse')} className="block hover:text-emerald-600">Cho Thuê Shophouse Khối Đế</button>
                      <button onClick={() => handleNavFilter('rent', 'thap-tang', 'biet-thu-song-lap')} className="block hover:text-emerald-600">Cho Thuê Biệt Thự Nguyên Căn</button>
                    </div>
                  </div>
                </div>
              )}
      ...
      {/* 3 Sub-Tabs Bar - 3 ô chữ nhật gọn gàng: Mua bán, Cho thuê BĐS, Dịch vụ cư dân */}
      <div className="bg-slate-100/90 dark:bg-slate-950/90 border-t border-slate-200/80 dark:border-slate-800/80 py-1.5 px-2 sm:px-3">
        <div className="max-w-3xl mx-auto grid grid-cols-3 gap-1 sm:gap-3">
          <button
            onClick={() => handleNavFilter('sale', 'all')}
            className={`py-1.5 px-1 sm:px-3 rounded-lg text-[10px] sm:text-xs font-bold transition flex items-center justify-center gap-0.5 sm:gap-1.5 border shadow-2xs ${
              currentTab === 'sale'
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 pt-2 pb-6 space-y-2 shadow-lg">
          ...
          {currentUser && (
            ...
            >
              <Zap className="w-4 h-4 mr-2" /> Quản Lý Tin & Up Tin
            </button>
          )}
          <button
            onClick={() => {
