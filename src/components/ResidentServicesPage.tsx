...
export const ResidentServicesPage: React.FC<ResidentServicesPageProps> = ({
  ...
  const displayServicesByTab = useMemo(() => {
    // GIAN HÀNG CHUNG: hiển thị TẤT CẢ dịch vụ cư dân (kể cả chưa KYC)
    return prioritizeByArea(filteredServices, getStoredAreaKey(), (s: any) => s.project);
  }, [filteredServices, storeVerificationTab]);

  // Filtered Stores by Verification Tab
  const displayStoresByTab = useMemo(() => {
    const list = stores.filter(st => {
      ...
      // GIAN HÀNG CHUNG: hiển thị tất cả gian hàng cư dân
      return true;
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      ...
      <PageBanner
        pageKey="dich-vu"
        fallbackTitle="DỊCH VỤ CƯ DÂN NỘI KHU"
        fallbackSubtitle="Sửa chữa, giặt là, giao đồ ăn và tiện ích 24/7"
        fallbackImage="/images/demo/ad-service.jpg"
        newsIndustry="dich-vu"
      />
          ...
          <div className="bg-white dark:bg-slate-900 p-2 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <div className="relative flex-1 w-full space-y-1.5">
                ...
                {/* Hot Service Suggestion Tags - đã bỏ gợi ý theo yêu cầu */}
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 p-1.5 sm:p-2.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:border-emerald-400 transition cursor-pointer text-center sm:text-left"
                >
                  <div className={`p-1.5 rounded-xl shrink-0 ${item.bg} ${item.color}`}>
                    <ItemIcon className="w-4 h-4" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-[10px] sm:text-xs font-bold leading-tight line-clamp-2">{item.label}</div>
                  </div>
                </button>
              );
            })}
            ...
            <button
              ...
              className={`flex flex-col sm:flex-row items-center gap-1 sm:gap-2.5 p-1.5 sm:p-2.5 rounded-2xl border transition cursor-pointer text-center sm:text-left ${
            ...
            >
              <div className={`p-1.5 rounded-xl shrink-0 ${selectedCategory === 'all' ? 'bg-white/20 text-white' : 'bg-emerald-500/10 text-emerald-500'}`}>
                <Wrench className="w-4 h-4" />
              </div>
              <div className="min-w-0">
                <div className="text-[10px] sm:text-xs font-bold leading-tight line-clamp-2">Tất Cả Dịch Vụ</div>
                <div className="text-[9px] opacity-70 truncate">Tổng {services.length} bài</div>
              </div>
            </button>

            {RESIDENT_SERVICE_CATEGORIES.filter(cat => !cat.name.includes('Thi Công Xây Lắp') && !cat.name.includes('Taxi Cư Dân')).map(cat => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col sm:flex-row items-center gap-1 sm:gap-2 p-1.5 sm:p-2.5 rounded-2xl border transition cursor-pointer text-center sm:text-left ${
                    isActive
                      ? 'bg-emerald-600 text-white border-emerald-500 shadow-md font-black'
                      : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-emerald-400'
                  }`}
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-[10px] sm:text-xs font-bold leading-tight line-clamp-2">{cat.name}</div>
                    <div className="text-[9px] opacity-75 truncate">{count} thợ</div>
                  </div>
                  {industryRule?.isStrictMandatory && (
                    <span className={`absolute top-1 right-1 w-2 h-2 rounded-full shrink-0 ${isActive ? 'bg-amber-300' : 'bg-emerald-500'}`} title="Yêu cầu giấy phép ngành nghề" />
                  )}
                </button>
              );
            })}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 sm:p-5 shadow-xl space-y-4">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <h2 className="text-sm sm:text-lg font-black text-slate-900 dark:text-white tracking-tight leading-snug">
                    🏪 GIAN HÀNG CHUNG CƯ DÂN
                  </h2>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-snug mt-0.5">
                    Gian hàng chung: hiển thị tất cả dịch vụ &amp; gian hàng của cư dân, phân loại theo ngành nghề.
                  </p>
                </div>
            ...
            <div className="p-2 rounded-xl border text-[11px] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 bg-emerald-500/10 border-emerald-500/20 text-emerald-800 dark:text-emerald-300">
              <div className="flex items-center gap-1.5 min-w-0">
                ...
                <span className="font-bold leading-snug">
                  Gian hàng chung cư dân — hiển thị tất cả gian hàng &amp; thợ dịch vụ. Ưu tiên gian hàng đã xác minh KYC.
                </span>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
              ...
              {/* View Mode Buttons */}
              <div className="grid grid-cols-4 gap-1 sm:flex sm:items-center sm:gap-0.5 sm:bg-slate-100 dark:sm:bg-slate-800 sm:p-0.5 sm:rounded-lg text-xs">
                <button
                  onClick={() => { setViewMode('grid-2col'); localStorage.setItem('hb_resident_services_view_mode', 'grid-2col'); }}
                  className={`px-1 sm:px-2 py-1.5 rounded-xl sm:rounded-md text-[10px] font-bold transition flex flex-col sm:flex-row items-center gap-0.5 sm:gap-1 border sm:border-0 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 sm:bg-transparent cursor-pointer ${viewMode === 'grid-2col' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'}`}
                >
                ...
                <button
                  ...
                  className={`px-1 sm:px-2 py-1.5 rounded-xl sm:rounded-md text-[10px] font-bold transition flex flex-col sm:flex-row items-center gap-0.5 sm:gap-1 border sm:border-0 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 sm:bg-transparent cursor-pointer ${viewMode === 'card-1col' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'}`}
                >
                ...
                <button
                  ...
                  className={`px-1 sm:px-2 py-1.5 rounded-xl sm:rounded-md text-[10px] font-bold transition flex flex-col sm:flex-row items-center gap-0.5 sm:gap-1 border sm:border-0 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 sm:bg-transparent cursor-pointer ${viewMode === 'list-row' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'}`}
                >
                ...
                <button
                  ...
                  className={`px-1 sm:px-2 py-1.5 rounded-xl sm:rounded-md text-[10px] font-bold transition flex flex-col sm:flex-row items-center gap-0.5 sm:gap-1 border sm:border-0 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 sm:bg-transparent cursor-pointer ${viewMode === 'grid-3col' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'}`}
                >
