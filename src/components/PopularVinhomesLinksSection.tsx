...
export const PopularVinhomesLinksSection: React.FC<PopularVinhomesLinksSectionProps> = ({
  ...
  return (
    <section className="bg-ink-950 text-ink-300 py-12 px-4 sm:px-6 lg:px-8 border-t border-ink-800">
      <div className="max-w-7xl mx-auto space-y-8">
        
        {/* Section Header Title */}
        <div className="flex items-center space-x-2 border-b border-ink-800 pb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-xs">
          ...
          <div className="space-y-3">
            <h3 className="font-extrabold text-sm text-white uppercase tracking-wider flex items-center gap-1.5 text-brand-400 border-b border-ink-800 pb-2">
              <ShoppingBag className="w-4 h-4 text-brand-500" />
              <span>Chủ đề & Dịch vụ cư dân</span>
            </h3>
            <ul className={showAll ? 'space-y-2 text-ink-400' : 'space-y-2 text-ink-400 hidden md:block'}>
              {[
                { label: ' Quán ăn & Thực phẩm F&B Cư Dân', tab: 'services' },
                { label: ' Chợ Cư Dân - Hải Sản & Đồ Tươi Sạch', tab: 'services' },
                { label: ' Thi công nội thất & Sửa chữa điện nước', tab: 'services' },
                ...
                { label: ' Đăng bài sản phẩm / Dịch vụ cư dân', tab: 'post-property' }
              ].map((item, idx) => (
                <li key={idx}>
                  <button
                    onClick={() => handleLinkClick(item.tab)}
                    className="hover:text-brand-400 transition flex items-center gap-1.5 group text-left cursor-pointer"
                  >
                    <ChevronRight className="w-3 h-3 text-ink-600 group-hover:text-brand-400 transition" />
          ...
          <div className="space-y-3">
            ...
            <ul className={showAll ? 'space-y-2 text-ink-400' : 'space-y-2 text-ink-400 hidden md:block'}>
              {[
                ...
                { label: ' Group Cư Dân Ocean Park 1 (Gia Lâm)', tab: 'projects', project: 'ocean-park-1' },
                ...
                { label: ' Group Cư Dân Vinhomes Hạ Long Xanh', tab: 'projects', project: 'ha-long-xanh' },
          ...
          <div className="space-y-3">
            <h3 className="font-extrabold text-sm text-white uppercase tracking-wider flex items-center gap-1.5 text-sky-400 border-b border-ink-800 pb-2">
            ...
            <ul className={showAll ? 'space-y-2 text-ink-400' : 'space-y-2 text-ink-400 hidden md:block'}>
              {[
                ...
                { label: ' Cho thuê biệt thự & shophouse kinh doanh', tab: 'rent', project: 'ocean-park-2' },
                ...
                { label: ' Đăng tin bán / Cho thuê nhà đất', tab: 'post-property' }
              ].map((item, idx) => (
                <li key={idx}>
                  <button
                    onClick={() => handleLinkClick(item.tab, item.project as ProjectCategory)}
                  ...
                  >
                    <ChevronRight className="w-3 h-3 text-ink-600 group-hover:text-sky-400 transition" />
                    <span>{item.label}</span>
                  </button>
                </li>
              ))}
        ...
        <button
          ...
          className="md:hidden w-full py-2 rounded-xl border border-ink-800 text-xs font-bold text-brand-400 hover:bg-ink-900 transition"
        >
