...
export const AiUrlTrackerModal: React.FC<AiUrlTrackerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-ink-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl max-w-3xl w-full p-6 sm:p-8 space-y-6 shadow-2xl relative">
        <button
          onClick={onClose}
        ...
        {/* Modal Header */}
        <div className="space-y-1.5 border-b border-ink-100 dark:border-ink-800 pb-4">
        ...
        <form onSubmit={handleRewrite} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 bg-ink-50 dark:bg-ink-950 border border-ink-200 dark:border-ink-800 rounded-xl text-ink-900 dark:text-white font-bold"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 bg-ink-50 dark:bg-ink-950 border border-ink-200 dark:border-ink-800 rounded-xl text-ink-900 dark:text-white"
                />
              </div>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3.5 bg-brand-500 hover:bg-brand-600 text-ink-950 font-black rounded-2xl text-xs uppercase tracking-wider transition shadow-lg flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
                <RefreshCw className="w-4 h-4 animate-spin text-ink-950" />
        ...
        {generatedArticle && (
          <div className="bg-ink-50 dark:bg-ink-950/90 border border-ink-200 dark:border-ink-800 rounded-3xl p-5 space-y-4 text-xs">
            <div className="flex items-center justify-between border-b border-ink-200 dark:border-ink-800 pb-3">
              <span className="text-xs font-black text-brand-500 flex items-center gap-1.5 uppercase">
                <CheckCircle2 className="w-4 h-4" />
                ĐÃ HOÀN THÀNH BIÊN SOẠN AI (CHỐNG VI PHẠM BẢN QUYỀN 100%)
              </span>
              <span className="px-2.5 py-1 bg-brand-500 text-ink-950 rounded-lg text-[10px] font-black">
                SEO Score: 99.8%
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-ink-600 dark:text-ink-300 font-medium bg-white dark:bg-ink-900 p-3 rounded-xl border border-ink-200 dark:border-ink-800">
            ...
            {generatedArticle.faqQA && generatedArticle.faqQA.length > 0 && (
              <div className="p-3 bg-brand-500/10 border border-brand-500/30 rounded-2xl space-y-2">
                <span className="text-[11px] font-black text-brand-500 uppercase flex items-center gap-1">
                  <BookOpen className="w-3.5 h-3.5" />
                  BỘ Q&A TỰ ĐỘNG TRÍCH XUẤT CHO KNOWLEDGE BASE
                </span>
                {generatedArticle.faqQA.map((q, idx) => (
                  <div key={idx} className="bg-white dark:bg-ink-900 p-2.5 rounded-xl text-[11px] space-y-1">
                    <p className="font-extrabold text-ink-900 dark:text-white">Q: {q.question}</p>
                    <p className="text-ink-600 dark:text-ink-300">A: {q.answer}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="pt-2 flex items-center justify-end gap-3">
              <button
                ...
                className="px-6 py-3 bg-brand-600 hover:bg-brand-700 text-white font-black text-xs rounded-xl transition shadow-lg uppercase tracking-wider flex items-center gap-2"
              >
