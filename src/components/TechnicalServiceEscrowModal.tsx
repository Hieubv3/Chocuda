...
export const TechnicalServiceEscrowModal: React.FC<TechnicalServiceEscrowModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-ink-900/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-ink-200 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        
        {/* Header Bar */}
        <div className="p-4 sm:p-5 bg-gradient-to-r from-brand-900 via-teal-900 to-ink-900 text-white flex items-center justify-between border-b border-brand-800/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-brand-500/20 border border-brand-400/30 flex items-center justify-center text-brand-300 shadow-inner">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-brand-500/30 text-brand-300 text-xs px-2.5 py-0.5 rounded-full border border-brand-400/30 font-medium">
                  Bảo Vệ Quyền Lợi 100%
                </span>
              </div>
              <p className="text-xs sm:text-sm text-brand-200/80 mt-0.5">
        ...
        <div className="bg-ink-900 text-ink-100 p-3 sm:p-4 text-xs border-b border-ink-800">
          <div className="max-w-4xl mx-auto space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-xs font-medium">
                <span className="bg-brand-900/60 text-brand-200 px-2 py-0.5 rounded border border-brand-700/50">1. Đặt lịch / Yêu cầu</span>
                <ChevronRight className="w-3.5 h-3.5 text-ink-500 hidden sm:inline" />
                <span className="bg-brand-900/60 text-brand-200 px-2 py-0.5 rounded border border-brand-700/50">2. Kết nối trực tiếp</span>
                <ChevronRight className="w-3.5 h-3.5 text-ink-500 hidden sm:inline" />
                <span className="bg-brand-900/60 text-brand-200 px-2 py-0.5 rounded border border-brand-700/50">3. Lưu Nhật Ký Lịch Sử</span>
                <ChevronRight className="w-3.5 h-3.5 text-ink-500 hidden sm:inline" />
                <span className="bg-brand-500/20 text-brand-300 px-2 py-0.5 rounded border border-brand-500/40 font-bold">4. Giải ngân 100% (Phí Sàn 0%)</span>
              </div>
            </div>

            <div className="bg-brand-500/10 border border-brand-500/30 rounded-lg p-2.5 text-brand-200 text-xs flex items-start gap-2">
              <AlertCircle className="w-4 h-4 text-brand-400 shrink-0 mt-0.5" />
              <div>
                <strong> Thông báo Pháp lý & Miễn trừ Trách nhiệm:</strong> Sàn chocudan24h.com là nền tảng kết nối thông tin trực tiếp giữa Khách hàng và Đơn vị dịch vụ/Thợ kỹ thuật. Sàn <u>KHÔNG thu % phí giao dịch</u> và <u>KHÔNG chịu trách nhiệm pháp lý</u> đối với chất lượng thi công, hợp đồng cá nhân hay thỏa thuận thanh toán giữa các bên. Đơn đặt hàng trên hệ thống chỉ phục vụ lưu trữ nhật ký lịch sử kết nối.
              </div>
            </div>
          </div>
        </div>
        ...
        {statusMessage && (
          <div className={`p-3 text-sm flex items-center justify-between gap-2 ${
            statusMessage.type === 'success' ? 'bg-brand-50 text-brand-800 border-b border-brand-200' :
            ...
            <div className="flex items-center gap-2 font-medium">
              {statusMessage.type === 'success' ? <CheckCircle2 className="w-5 h-5 text-brand-600 shrink-0" /> : <AlertCircle className="w-5 h-5 text-rose-600 shrink-0" />}
              <span>{statusMessage.text}</span>
            </div>
            <button onClick={() => setStatusMessage(null)} className="text-ink-400 hover:text-ink-600">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-ink-50/50">
          ...
          {/* Navigation Tabs */}
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6 bg-white p-1.5 rounded-xl border border-ink-200 shadow-sm">
            ...
            <button 
              onClick={fetchData}
              className="px-3 py-1.5 text-xs text-ink-500 hover:text-ink-800 flex items-center gap-1.5 rounded-lg border border-ink-200 bg-white hover:bg-ink-50"
            >
          ...
          {activeTab === 'orders' && (
            <div className="space-y-4">
              ...
              {orders.length === 0 ? (
                <div className="bg-white p-8 rounded-2xl border border-ink-200 text-center">
                  <Wrench className="w-12 h-12 text-ink-300 mx-auto mb-3" />
                  <p className="text-ink-600 font-semibold">Chưa có đơn dịch vụ kỹ thuật nào.</p>
                  <p className="text-xs text-ink-400 mt-1">Bấm &quot;Tạo Đơn Tạm Giữ Mới&quot; để đặt thợ sửa thang máy, điện nước hoặc thiết bị smarthome.</p>
                  <button 
                    onClick={() => setActiveTab('create_order')}
                    className="mt-4 px-4 py-2 bg-brand-600 text-white rounded-xl font-semibold text-sm hover:bg-brand-700 transition-colors inline-flex items-center gap-2"
                  >
                    <Lock className="w-4 h-4" />
                ...
                <div className="grid grid-cols-1 gap-4">
                  {orders.map((ord) => (
                    <div key={ord.id} className="bg-white rounded-2xl border border-ink-200 shadow-sm overflow-hidden hover:border-brand-300 transition-all">
                      <div className="p-4 sm:p-5">
                        
                        {/* Order Top Line */}
                        <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-ink-100">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold bg-ink-100 text-ink-700 px-2.5 py-1 rounded-lg border border-ink-200">
                              {ord.orderCode}
                            </span>
                            <span className="text-xs text-ink-500 font-medium">{ord.subCategory}</span>
                          </div>

                          {/* Status Badge */}
                          <div>
                            {ord.status === 'completed_released' ? (
                              <span className="inline-flex items-center gap-1.5 bg-brand-100 text-brand-800 text-xs px-3 py-1 rounded-full font-bold border border-brand-300">
                                <CheckCircle2 className="w-3.5 h-3.5 text-brand-600" />
                                ĐÃ GIẢI NGÂN TỰ ĐỘNG VÀO VÍ THỢ
                              </span>
                            ) : ord.status === 'inspection_submitted' ? (
                              <span className="inline-flex items-center gap-1.5 bg-brand-100 text-brand-800 text-xs px-3 py-1 rounded-full font-bold border border-brand-300 animate-pulse">
                        ...
                        <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="md:col-span-2 space-y-2">
                            <h5 className="font-bold text-ink-900 text-base sm:text-lg leading-snug">
                              {ord.serviceTitle}
                            </h5>
                            
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-ink-600">
                            ...
                            {ord.note && (
                              <div className="bg-ink-50 p-2.5 rounded-xl border border-ink-200 text-xs text-ink-700">
                            ...
                            {((ord.imagesBefore && ord.imagesBefore.length > 0) || (ord.imagesAfter && ord.imagesAfter.length > 0)) && (
                              <div className="flex items-center gap-3 pt-2">
                                {ord.imagesBefore && ord.imagesBefore[0] && (
                                  <div>
                                    <span className="text-[10px] text-ink-400 block mb-1">Ảnh trước thi công:</span>
                                    <img loading="lazy" src={ord.imagesBefore[0]} alt="Before" className="w-16 h-16 rounded-lg object-cover border border-ink-200" />
                                  </div>
                                )}
                                {ord.imagesAfter && ord.imagesAfter[0] && (
                                  <div>
                                    <span className="text-[10px] text-brand-600 font-bold block mb-1">Ảnh đã nghiệm thu:</span>
                                    <img loading="lazy" src={ord.imagesAfter[0]} alt="After" className="w-16 h-16 rounded-lg object-cover border border-brand-300" />
                                  </div>
                                )}
                              </div>
                            )}
                          ...
                          <div className="bg-gradient-to-br from-brand-50 to-teal-50/50 p-4 rounded-xl border border-brand-200 flex flex-col justify-between">
                            <div>
                              <div className="text-xs text-ink-500 font-medium mb-1">Giá trị hợp đồng dịch vụ:</div>
                              ...
                              <div className="mt-3 pt-3 border-t border-brand-200/60 space-y-1 text-xs">
                                <div className="flex justify-between text-ink-600">
                                  <span>Phí quản lý hệ thống (5%):</span>
                                  <span className="font-semibold text-ink-700">-{ord.platformFee.toLocaleString('vi-VN')}đ</span>
                                </div>
                                <div className="flex justify-between text-brand-800 font-bold pt-1">
                            ...
                            <div className="mt-4 pt-3 border-t border-brand-200/60">
                              {ord.status !== 'completed_released' ? (
                                <button
                                  ...
                                  className="w-full py-2.5 px-3 bg-gradient-to-r from-brand-600 to-teal-600 hover:from-brand-700 hover:to-teal-700 text-white rounded-xl font-bold text-xs shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-1.5"
                                >
                                  <CheckCircle2 className="w-4 h-4" />
                                  <span>BẤM NGHIỆM THU &amp; GIẢI NGÂN</span>
                                </button>
                              ) : (
                                <div className="text-center p-2 bg-brand-100 text-brand-800 rounded-lg text-xs font-bold flex items-center justify-center gap-1">
          ...
          {activeTab === 'wallet' && (
            <div className="space-y-6">
              ...
              {/* Wallet Summary Card */}
              <div className="bg-gradient-to-br from-ink-900 via-brand-950 to-teal-900 text-white rounded-2xl p-6 shadow-xl border border-brand-500/30">
                <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-brand-800/60">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-brand-500/20 border border-brand-400/30 flex items-center justify-center text-brand-300">
                      <Wallet className="w-6 h-6" />
                    </div>
                    <div>
                      <span className="text-xs text-brand-300 uppercase tracking-wider font-semibold">Ví Tự Động chocudan24h</span>
                      <h4 className="text-xl font-extrabold text-white">{currentUser?.name || 'Cư Dân Vinhomes'}</h4>
                    </div>
                  </div>

                  <span className="bg-brand-500/20 border border-brand-400/30 text-brand-300 text-xs px-3 py-1 rounded-full font-bold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
                  ...
                  <div className="bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                    <div className="text-xs text-brand-200/80 font-medium mb-1 flex items-center gap-1.5">
                      <DollarSign className="w-4 h-4 text-brand-400" />
                      <span>Số Dư Khả Dụng</span>
                    </div>
                    <div className="text-2xl font-black text-white">
                      {(wallet?.availableBalance || 0).toLocaleString('vi-VN')} <span className="text-xs font-normal text-brand-200">đ</span>
                    </div>
                    <p className="text-[10px] text-brand-300/70 mt-1">Dùng thanh toán dịch vụ hoặc rút về NH</p>
                  </div>
                  ...
                  <div className="bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                    ...
                    <div className="text-2xl font-black text-brand-300">
                      {(wallet?.escrowLockedBalance || 0).toLocaleString('vi-VN')} <span className="text-xs font-normal text-brand-200">đ</span>
                    </div>
                    <p className="text-[10px] text-brand-300/70 mt-1">Thuật toán khóa an toàn cho các đơn kỹ thuật</p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-ink-200 shadow-sm space-y-4">
                  <div className="flex items-center gap-2 border-b border-ink-100 pb-3">
                    ...
                    <h5 className="font-bold text-ink-900 text-base">Nạp Tiền Ví Cư Dân Qua VietQR</h5>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Số tiền muốn nạp (VNĐ):</label>
                    <input 
                      type="number"
                      value={depositAmount}
                      onChange={(e) => setDepositAmount(e.target.value)}
                      placeholder="Ví dụ: 2000000"
                      className="w-full px-3 py-2 border border-ink-300 rounded-xl font-bold text-ink-900 text-sm focus:ring-2 focus:ring-brand-500"
                    />
                  </div>

                  {/* VietQR Quick Code Box */}
                  <div className="bg-ink-50 p-3.5 rounded-xl border border-ink-200 text-center space-y-2">
                    <p className="text-xs text-ink-600 font-medium">Mã VietQR Tự Động Nhận Tiền Nạp chocudan24h:</p>
                    <img loading="lazy" 
                      src={`https://img.vietqr.io/image/MB-3028031988-compact2.png?amount=${depositAmount}&addInfo=NAP%20VI%20${userId}`}
                      alt="VietQR Deposit"
                      className="w-44 h-44 mx-auto rounded-lg shadow border border-ink-200 object-contain"
                    />
                    <div className="text-[11px] text-ink-500">
                      Ngân hàng: <strong>MBBank</strong> | STK: <strong className="text-brand-700">3028031988</strong> | Chủ TK: <strong>BUI VAN HIEU</strong>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-sm shadow transition-all flex items-center justify-center gap-2"
                  >
                ...
                <div className="bg-white p-5 rounded-2xl border border-ink-200 shadow-sm space-y-4 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 border-b border-ink-100 pb-3 mb-4">
                      <ArrowUpRight className="w-5 h-5 text-sky-600" />
                      <h5 className="font-bold text-ink-900 text-base">Rút Tiền Về Ngân Hàng Tự Động (Cho Thợ)</h5>
                    </div>
                    ...
                    <div className="bg-sky-50/80 p-3.5 rounded-xl border border-sky-200 text-xs space-y-1 mb-4">
                      ...
                      {wallet?.bankDetails?.accountNumber ? (
                        <div className="text-ink-700 font-medium space-y-0.5">
                          <p>• Ngân hàng: <strong className="text-ink-900">{wallet.bankDetails.bankName}</strong></p>
                          <p>• Số tài khoản: <strong className="text-sky-700">{wallet.bankDetails.accountNumber}</strong></p>
                          <p>• Chủ tài khoản: <strong className="text-ink-900">{wallet.bankDetails.accountHolder}</strong></p>
                        </div>
                      ) : (
                    ...
                    <div>
                      <label className="block text-xs font-bold text-ink-700 mb-1">Số tiền muốn rút về Ngân hàng (VNĐ):</label>
                      <input 
                        type="number"
                        value={withdrawAmount}
                        onChange={(e) => setWithdrawAmount(e.target.value)}
                        placeholder="Ví dụ: 1000000"
                        className="w-full px-3 py-2 border border-ink-300 rounded-xl font-bold text-ink-900 text-sm focus:ring-2 focus:ring-sky-500"
                      />
                    </div>
                  </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-ink-200 shadow-sm space-y-3">
                <h5 className="font-bold text-ink-900 text-sm flex items-center gap-2">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs text-left border-collapse">
                    <thead>
                      <tr className="bg-ink-100 text-ink-700 border-b border-ink-200">
                    ...
                    <tbody className="divide-y divide-ink-100">
                      {transactions.map((tx) => (
                        <tr key={tx.id} className="hover:bg-ink-50">
                          <td className="p-2.5 font-mono text-ink-600 font-bold">{tx.referenceCode || tx.id}</td>
                          <td className="p-2.5">
                            {tx.type === 'escrow_hold' ? (
                              <span className="bg-brand-100 text-brand-800 px-2 py-0.5 rounded font-bold">Tạm Giữ Escrow</span>
                            ) : tx.type === 'escrow_release' ? (
                              <span className="bg-brand-100 text-brand-800 px-2 py-0.5 rounded font-bold">Giải Ngân Về Ví</span>
                          ...
                          <td className="p-2.5 text-ink-800 font-medium">{tx.description}</td>
                          <td className="p-2.5 font-bold text-ink-900">
                            {tx.amount.toLocaleString('vi-VN')}đ
                          </td>
                          <td className="p-2.5 text-ink-400">{tx.createdAt}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeTab === 'create_order' && (
            <div className="max-w-2xl mx-auto bg-white p-6 rounded-2xl border border-ink-200 shadow-sm space-y-4">
              ...
              <form onSubmit={handleCreateOrder} className="space-y-4 text-xs">
                <div>
                  <label className="block font-bold text-ink-700 mb-1">Tên Dịch Vụ Kỹ Thuật:</label>
                  <input 
                    type="text"
                    value={serviceTitle}
                    onChange={(e) => setServiceTitle(e.target.value)}
                    required
                    className="w-full px-3 py-2 border border-ink-300 rounded-xl font-medium text-ink-900 text-sm focus:ring-2 focus:ring-brand-500"
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-bold text-ink-700 mb-1">Giá Trị Thỏa Thuận (VNĐ):</label>
                    <input 
                      type="number"
                      value={agreedPrice}
                      onChange={(e) => setAgreedPrice(e.target.value)}
                      required
                      className="w-full px-3 py-2 border border-ink-300 rounded-xl font-bold text-brand-700 text-sm focus:ring-2 focus:ring-brand-500"
                    />
                  </div>

                  <div>
                    <label className="block font-bold text-ink-700 mb-1">Số Ngày Bảo Hành Cam Kết:</label>
                    <input 
                      type="number"
                      value={warrantyDays}
                      onChange={(e) => setWarrantyDays(Number(e.target.value))}
                      className="w-full px-3 py-2 border border-ink-300 rounded-xl font-medium text-ink-900 text-sm"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-bold text-ink-700 mb-1">Tên Thợ / Đội Kỹ Thuật:</label>
                    <input 
                      type="text"
                      value={techName}
                  ...
                  <div>
                    <label className="block font-bold text-ink-700 mb-1">Số Điện Thoại Thợ:</label>
                    <input 
                      type="text"
                      value={techPhone}
                ...
                <div>
                  <label className="block font-bold text-ink-700 mb-1">Địa Chỉ Thi Công Căn Hộ / Biệt Thự:</label>
                  <input 
                    type="text"
                ...
                <div>
                  <label className="block font-bold text-ink-700 mb-1">Ghi Chú Công Việc Chi Tiết:</label>
                  <textarea 
                    rows={3}
                    value={orderNote}
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-brand-600 to-teal-600 hover:from-brand-700 hover:to-teal-700 text-white rounded-xl font-bold text-sm shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
                >
                  <Lock className="w-4 h-4" />
          ...
          {activeTab === 'bank_linking' && (
            <div className="max-w-xl mx-auto bg-white p-6 rounded-2xl border border-ink-200 shadow-sm space-y-4">
              <div className="border-b border-ink-100 pb-3">
                <h4 className="text-lg font-bold text-ink-900 flex items-center gap-2">
              ...
              <div className="space-y-4 text-xs">
                <div>
                  <label className="block font-bold text-ink-700 mb-1">Tên Ngân Hàng:</label>
                  <input 
                    type="text"
                    value={bankName}
                ...
                <div>
                  <label className="block font-bold text-ink-700 mb-1">Số Tài Khoản Ngân Hàng:</label>
                  <input 
                    type="text"
                    value={accountNumber}
                    onChange={(e) => setAccountNumber(e.target.value)}
                    placeholder="Ví dụ: 3028031988"
                    className="w-full px-3 py-2 border border-ink-300 rounded-xl font-bold text-sky-700 text-sm"
                  />
                </div>

                <div>
                  <label className="block font-bold text-ink-700 mb-1">Họ Về Tên Chủ Tài Khoản (Viết Hoa Không Dấu):</label>
                  <input 
                    type="text"
                    value={accountHolder}
                    onChange={(e) => setAccountHolder(e.target.value)}
                    placeholder="Ví dụ: BUI VAN HIEU"
                    className="w-full px-3 py-2 border border-ink-300 rounded-xl font-bold text-ink-900 text-sm"
                  />
                </div>
        ...
        {/* Modal Footer */}
        <div className="p-4 bg-ink-100 border-t border-ink-200 flex items-center justify-between text-xs text-ink-500">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-ink-800 text-white rounded-xl font-semibold hover:bg-ink-900 transition-colors"
          >
            Đóng
          </button>
        </div>
