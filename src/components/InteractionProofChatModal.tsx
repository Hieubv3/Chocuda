...
export const InteractionProofChatModal: React.FC<InteractionProofChatModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[90] overflow-y-auto bg-ink-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4">
      <div className="relative w-full max-w-lg bg-ink-900 border border-ink-800 rounded-3xl shadow-2xl overflow-hidden text-white flex flex-col max-h-[90vh]">
        
        {/* Modal Header */}
        <div className="bg-gradient-to-r from-ink-950 via-indigo-950 to-ink-950 p-4 border-b border-ink-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-brand-500/20 text-brand-400 rounded-xl border border-brand-500/30">
              <MessageSquare className="w-5 h-5" />
            </div>
            <div>
              <span className="px-2 py-0.5 bg-brand-500/20 text-brand-400 font-extrabold text-[10px] rounded uppercase">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 text-xs flex-1">
          {sentSuccess ? (
            <div className="text-center py-6 space-y-4">
              <div className="w-16 h-16 bg-brand-500/20 text-brand-400 rounded-full flex items-center justify-center mx-auto border border-brand-500/40 animate-bounce">
                <CheckCircle2 className="w-10 h-10" />
              </div>

              <div className="space-y-1">
                <h3 className="text-lg font-black text-brand-300">
              ...
              <div className="bg-ink-800/80 p-3 rounded-2xl border border-ink-700 text-left space-y-1.5 text-[11px]">
                <div className="flex justify-between items-center text-ink-400">
                  <span>Hoạt động:</span>
                  <span className="font-bold text-brand-400">{currentAct.title}</span>
                </div>
                <div className="flex justify-between items-center text-ink-400">
                  <span>Trạng thái Admin Chat:</span>
                  <span className="font-bold text-brand-400"> Đã Duyệt Tự Động (+{earnedPoints} Up)</span>
                </div>
              </div>

              <button
                onClick={onClose}
                className="w-full py-3 bg-brand-500 hover:bg-brand-400 text-ink-950 font-black rounded-xl uppercase transition shadow-lg text-xs"
              >
            ...
            <form onSubmit={handleSubmitProof} className="space-y-4">
              ...
              {/* Select Activity */}
              <div>
                <label className="block font-extrabold text-brand-300 mb-2">
                      ...
                      <div
                        ...
                        className={`p-3 rounded-2xl border cursor-pointer transition flex items-center justify-between gap-3 ${
                          isSelected
                            ...
                            : 'bg-ink-800/60 border-ink-700/80 hover:bg-ink-800 text-ink-300'
                        }`}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-ink-400 line-clamp-1">{act.desc}</p>
                          </div>
                        </div>

                        <span className="px-2.5 py-1 bg-brand-500 text-ink-950 font-black text-[11px] rounded-xl shrink-0 shadow-xs">
                          +{act.points} Up
                        </span>
                      </div>
                    );
              ...
              <div className="space-y-3 pt-2 border-t border-ink-800">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 bg-ink-800 border border-ink-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-brand-500 text-xs"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <ImageIcon className="w-4 h-4 absolute left-3 top-3 text-ink-400" />
                    <input
                      type="text"
                      value={proofImage}
                      onChange={(e) => setProofImage(e.target.value)}
                      placeholder="https://..."
                      className="w-full pl-9 pr-3.5 py-2.5 bg-ink-800 border border-ink-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-brand-500 text-xs"
                    />
                  </div>
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={isSending}
                className="w-full py-3 bg-gradient-to-r from-brand-500 to-brand-400 hover:from-brand-400 hover:to-brand-300 text-ink-950 font-black rounded-2xl uppercase tracking-wider transition shadow-lg text-xs flex items-center justify-center gap-2 cursor-pointer active:scale-95"
              >
                {isSending ? (
