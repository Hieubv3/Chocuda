...
export const UserCvManagement: React.FC<UserCvManagementProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-teal-950 via-ink-900 to-brand-950 p-5 sm:p-6 rounded-3xl border border-teal-500/30 text-white shadow-xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 bg-teal-500 text-ink-950 font-black text-[10px] rounded-full uppercase tracking-wider">
                HỒ SƠ CV CƯ DÂN 24H
              </span>
              {cvProfile?.isLookingForJob ? (
                <span className="px-2.5 py-0.5 bg-brand-500/20 text-brand-400 border border-brand-500/30 text-[10px] font-bold rounded-full flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-brand-400 animate-pulse"></span>
                  Đang Tìm Việc & Mở Liên Hệ
                </span>
              ) : cvProfile ? (
                <span className="px-2.5 py-0.5 bg-ink-800 text-ink-400 text-[10px] font-bold rounded-full">
                  Tạm Dừng Tìm Việc
                </span>
              ) : null}
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {cvProfile ? (
              <button
                ...
                className="px-4 py-2.5 bg-gradient-to-r from-teal-500 to-brand-600 hover:from-teal-400 hover:to-brand-500 text-ink-950 font-black rounded-xl text-xs flex items-center gap-1.5 shadow-lg transition cursor-pointer"
              >
            ...
            <button
              onClick={fetchData}
              disabled={isLoading}
              className="p-2.5 bg-ink-800 hover:bg-ink-700 text-ink-300 rounded-xl transition cursor-pointer"
              title="Làm mới"
            >
      ...
      {cvProfile ? (
        <div className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 space-y-6 shadow-sm">
          {/* Header Info */}
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-6 border-b border-ink-100 dark:border-ink-800">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 bg-ink-100 dark:bg-ink-800 text-ink-600 dark:text-ink-300 text-[10px] font-bold rounded-md">
                ...
                <div className="flex items-center gap-3 text-xs text-ink-500 dark:text-ink-400 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap self-end md:self-auto">
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${
                  cvProfile.isLookingForJob
                    ? 'bg-brand-100 dark:bg-brand-950/60 text-brand-700 dark:text-brand-300 border border-brand-300 dark:border-brand-800'
                    : 'bg-ink-100 dark:bg-ink-800 text-ink-500 border border-ink-300 dark:border-ink-700'
                }`}
              >
                {cvProfile.isLookingForJob ? <ToggleRight className="w-4 h-4 text-brand-600" /> : <ToggleLeft className="w-4 h-4 text-ink-400" />}
              ...
              <Link
                ...
                target="_blank"
                className="px-3.5 py-2 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 dark:hover:bg-ink-700 text-ink-700 dark:text-ink-200 text-xs font-bold rounded-xl flex items-center gap-1.5 transition"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
            ...
            <div className="space-y-4">
              <h4 className="font-black text-ink-900 dark:text-white uppercase tracking-wider text-[11px] text-teal-600 dark:text-teal-400">
                Thông Tin Liên Hệ (Bảo Mật)
              </h4>
              <div className="bg-ink-50 dark:bg-ink-800/60 p-3.5 rounded-2xl border border-ink-100 dark:border-ink-700/60 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Số Điện Thoại:</span>
                  <span className="font-bold text-ink-900 dark:text-white">{cvProfile.phone}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Zalo:</span>
                  <span className="font-bold text-teal-600">{cvProfile.zalo || cvProfile.phone}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Email:</span>
                  <span className="font-bold text-ink-900 dark:text-white truncate max-w-[160px]">{cvProfile.email}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Địa chỉ hiện tại:</span>
                  <span className="font-medium text-ink-700 dark:text-ink-300 text-right">{cvProfile.currentAddress || 'Nội khu Vinhomes'}</span>
                </div>
              </div>

              <div>
                <h4 className="font-black text-ink-900 dark:text-white uppercase tracking-wider text-[11px] text-teal-600 dark:text-teal-400 mb-2">
                  Giới Thiệu Bản Thân
                </h4>
                <p className="text-ink-600 dark:text-ink-300 leading-relaxed bg-ink-50 dark:bg-ink-800/40 p-3 rounded-2xl">
            ...
            <div className="space-y-4 md:col-span-2">
              ...
              <div className="space-y-2.5">
                <span className="text-ink-500 font-bold block text-[11px]">Lịch Sử Công Tác:</span>
                {(cvProfile.workExperience && cvProfile.workExperience.length > 0) ? (
                  cvProfile.workExperience.map((exp, idx) => (
                    <div key={idx} className="p-3 bg-ink-50 dark:bg-ink-800/50 rounded-2xl border border-ink-100 dark:border-ink-700/60 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-black text-ink-900 dark:text-white text-xs">{exp.position}</span>
                      ...
                      {exp.description && <p className="text-ink-500 dark:text-ink-400 text-[11px] mt-1">{exp.description}</p>}
                    </div>
                  ))
                ) : (
                  <div className="p-3 text-ink-400 italic bg-ink-50 dark:bg-ink-800/30 rounded-xl">
        ...
        /* Empty State */
        <div className="text-center py-12 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-8 space-y-4 shadow-sm">
          ...
          <button
            onClick={() => setShowEditModal(true)}
            className="px-6 py-3 bg-gradient-to-r from-teal-500 to-brand-600 hover:from-teal-400 hover:to-brand-500 text-ink-950 font-black rounded-xl text-xs shadow-lg transition cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 space-y-4 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="divide-y divide-ink-100 dark:divide-ink-800">
            {applications.map((app) => (
              <div key={app.id} className="py-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                <div className="space-y-1">
                  <div className="font-black text-ink-900 dark:text-white text-sm">
                    {app.jobTitle}
                  </div>
                  <div className="text-ink-600 dark:text-ink-300 font-medium flex items-center gap-2">
                    <span className="text-teal-600 font-bold">{app.companyName}</span>
                    <span>•</span>
                    <span className="text-ink-400">{app.createdAt}</span>
                  </div>
                  {app.message && (
                    <p className="text-[11px] text-ink-500 italic">Lời nhắn: "{app.message}"</p>
                  )}
                </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-ink-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden my-8">
            <div className="bg-gradient-to-r from-teal-950 to-ink-900 p-5 text-white flex items-center justify-between border-b border-teal-500/30">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-teal-500 text-ink-950 rounded-xl font-bold">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-300">Nhập đầy đủ thông tin để thu hút các nhà tuyển dụng hàng đầu nội khu</p>
                </div>
              </div>

              <button
                onClick={() => setShowEditModal(false)}
                className="p-2 text-ink-400 hover:text-white rounded-full hover:bg-ink-800 transition cursor-pointer"
              >
            ...
            <form onSubmit={handleSaveCv} className="p-6 space-y-4 text-xs max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Họ và Tên *</label>
                  <input
                    type="text"
                    required
                    value={formData.fullName || ''}
                    onChange={e => setFormData({ ...formData, fullName: e.target.value })}
                    className="w-full px-3.5 py-2.5 bg-ink-50 dark:bg-ink-800 border border-ink-300 dark:border-ink-700 rounded-xl text-ink-900 dark:text-white font-medium"
                    placeholder="VD: Nguyễn Văn Nam"
                  />
                </div>

                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Vị Trí Ứng Tuyển Mong Muốn *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Số Điện Thoại Liên Hệ *</label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Zalo Liên Hệ</label>
                  <input
                    type="text"
                    value={formData.zalo || ''}
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Email</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Năm Sinh & Giới Tính</label>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      value={formData.birthYear || 1998}
                      onChange={e => setFormData({ ...formData, birthYear: parseInt(e.target.value, 10) || 1998 })}
                      className="w-1/2 px-3.5 py-2.5 bg-ink-50 dark:bg-ink-800 border border-ink-300 dark:border-ink-700 rounded-xl text-ink-900 dark:text-white font-medium"
                    />
                    <select
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Khu Đô Thị Hiện Tại *</label>
                  <select
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Mức Lương Kỳ Vọng</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Ngành Nghề Chính *</label>
                  <select
                ...
                <div>
                  <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Số Năm Kinh Nghiệm</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block font-bold text-ink-700 dark:text-ink-300 mb-1">Giới Thiệu Bản Thân & Điểm Mạnh</label>
                <textarea
                  rows={3}
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-ink-200 dark:border-ink-800">
                <button
                  type="button"
                  onClick={() => setShowEditModal(false)}
                  className="px-4 py-2.5 bg-ink-100 dark:bg-ink-800 text-ink-600 dark:text-ink-300 font-bold rounded-xl hover:bg-ink-200 transition cursor-pointer"
                >
                  Hủy Bỏ
                </button>
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-teal-500 to-brand-600 hover:from-teal-400 hover:to-brand-500 text-ink-950 font-black rounded-xl shadow-lg transition cursor-pointer"
                >
