...
export const OmnichannelBulkMarketingModal: React.FC<OmnichannelBulkMarketingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink-950/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
      <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden my-auto max-h-[92vh] flex flex-col animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="bg-ink-900 text-white p-4 sm:p-6 border-b border-ink-800 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-3 bg-gradient-to-tr from-brand-600 to-teal-500 rounded-2xl shadow-inner text-white">
              <Share2 className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-black text-white tracking-tight flex items-center gap-2">
                HỆ THỐNG MARKETING & NHẮN TIN HÀNG LOẠT
                <span className="text-[10px] bg-brand-500/20 text-brand-400 font-bold px-2 py-0.5 rounded border border-brand-500/40">
          ...
          <button
            onClick={onClose}
            className="p-2 text-ink-400 hover:text-white bg-ink-800 hover:bg-ink-700 rounded-2xl transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Content Body */}
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 text-ink-800 dark:text-ink-100 text-xs sm:text-sm">
          
          {/* STEP 1: Select Property to Promote */}
          <div className="bg-ink-50 dark:bg-ink-950/60 p-4 rounded-2xl border border-ink-200 dark:border-ink-800 space-y-3">
            <label className="font-extrabold text-ink-900 dark:text-white flex items-center gap-2 text-xs uppercase tracking-wide">
            ...
            <select
              ...
              className="w-full p-3 bg-white dark:bg-ink-900 border border-ink-300 dark:border-ink-700 rounded-xl font-bold text-ink-900 dark:text-white focus:outline-none focus:border-brand-500 shadow-xs"
            >
              {properties.map((p) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-ink-50 dark:bg-ink-950/60 p-4 rounded-2xl border border-ink-200 dark:border-ink-800 space-y-3">
              ...
              <div className="space-y-2">
                <label className="flex items-center space-x-2.5 p-2.5 bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 cursor-pointer hover:border-brand-500 transition">
                  <input
                    type="checkbox"
                  ...
                  <span className="text-[10px] text-ink-400 bg-ink-100 dark:bg-ink-800 px-2 py-0.5 rounded font-mono">1,250 SĐT</span>
                </label>
                ...
                <label className="flex items-center space-x-2.5 p-2.5 bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 cursor-pointer hover:border-brand-500 transition">
                  ...
                  <span className="text-[10px] text-ink-400 bg-ink-100 dark:bg-ink-800 px-2 py-0.5 rounded font-mono">480 SĐT</span>
                </label>
                ...
                <label className="flex items-center space-x-2.5 p-2.5 bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 cursor-pointer hover:border-brand-500 transition">
                  ...
                  <span className="text-[10px] text-ink-400 bg-ink-100 dark:bg-ink-800 px-2 py-0.5 rounded font-mono">850 SĐT</span>
                </label>
              </div>

              <div>
                <span className="text-[11px] font-bold text-ink-500 block mb-1">Thêm SĐT/Email thủ công (ngăn cách bởi dấu phẩy):</span>
                <input
                  type="text"
                  value={customRecipients}
                  onChange={(e) => setCustomRecipients(e.target.value)}
                  placeholder="0988xxx, email1@gmail.com..."
                  className="w-full p-2 bg-white dark:bg-ink-900 border border-ink-300 dark:border-ink-700 rounded-xl text-xs focus:outline-none focus:border-brand-500"
                />
              </div>
            </div>
            ...
            <div className="bg-ink-50 dark:bg-ink-950/60 p-4 rounded-2xl border border-ink-200 dark:border-ink-800 space-y-3">
              ...
              <div className="space-y-2">
                <label className="flex items-center space-x-2.5 p-2.5 bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 cursor-pointer hover:border-brand-500 transition">
                  ...
                  <span className="text-[10px] text-brand-600 font-bold bg-brand-50 dark:bg-brand-950/50 px-2 py-0.5 rounded">Khuyên Dùng</span>
                </label>
          ...
          <div className="bg-ink-50 dark:bg-ink-950/60 p-4 rounded-2xl border border-ink-200 dark:border-ink-800 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-ink-200 dark:border-ink-800 pb-3">
              <label className="font-extrabold text-ink-900 dark:text-white flex items-center gap-2 text-xs uppercase tracking-wide">
              ...
              <button
                ...
                className="bg-gradient-to-r from-brand-600 to-teal-600 hover:from-brand-500 hover:to-teal-500 text-white font-extrabold px-4 py-2 rounded-xl transition flex items-center justify-center gap-2 shadow-sm text-xs active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full p-3 bg-white dark:bg-ink-900 border border-ink-300 dark:border-ink-700 rounded-xl text-xs focus:outline-none focus:border-brand-500 font-sans"
                />
              </div>
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white dark:bg-ink-900 border border-ink-300 dark:border-ink-700 rounded-xl text-xs font-bold focus:outline-none focus:border-brand-500"
                />
                <textarea
                  rows={3}
                  value={emailTemplate}
          ...
          {sendLogs.length > 0 && (
            <div className="bg-ink-900 text-white p-4 rounded-2xl border border-ink-800 space-y-3 font-mono text-xs">
              <div className="flex items-center justify-between text-brand-400 font-bold border-b border-ink-800 pb-2">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {sendLogs.map((log) => (
                  <div key={log.id} className="flex items-center justify-between text-[11px] bg-ink-800/80 p-2 rounded border border-ink-700/50">
                    <span className="text-ink-300 font-bold">[{log.channel}]  {log.target}</span>
                    <span className="text-brand-400 flex items-center gap-1 font-extrabold">
        ...
        {/* Footer Actions */}
        <div className="bg-ink-100 dark:bg-ink-950 p-4 border-t border-ink-200 dark:border-ink-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-3 w-full sm:w-auto">
            <button
              onClick={onClose}
              className="flex-1 sm:flex-none px-5 py-2.5 text-xs font-bold text-ink-600 dark:text-ink-300 hover:bg-ink-200 dark:hover:bg-ink-800 rounded-xl transition"
            >
              Đóng
            </button>
            ...
            <button
              ...
              disabled={isSending}
              className="flex-1 sm:flex-none px-6 py-2.5 text-xs font-black bg-brand-600 hover:bg-brand-500 active:bg-brand-700 text-white rounded-xl shadow-lg transition flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
            >
              {isSending ? (
                <>
