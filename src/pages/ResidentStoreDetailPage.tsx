  ...
  if (!store) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-4">
        <h1 className="text-2xl font-black text-ink-900 dark:text-white">Không tìm thấy gian hàng cư dân</h1>
        <p className="text-xs text-ink-500">Gian hàng có thể đang cập nhật hoặc tạm đóng cửa.</p>
        <Link to="/dich-vu-cu-dan" className="inline-block px-6 py-2.5 bg-brand-500 text-ink-950 font-bold rounded-xl text-xs">
  ...
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center text-xs font-semibold text-ink-500 dark:text-ink-400 overflow-x-auto whitespace-nowrap gap-1.5">
            ...
            {selectedProduct && (
              <>
                <ChevronRight className="w-3.5 h-3.5 shrink-0 text-ink-400" />
                <span className="text-brand-600 dark:text-brand-400 font-black truncate">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        <div className="relative rounded-3xl overflow-hidden bg-ink-950 border border-ink-200 dark:border-ink-800 shadow-xl">
          ...
          <div className="p-6 sm:p-8 relative -mt-16 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 bg-ink-900/90 backdrop-blur-md">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
              <img loading="lazy"
                src={store.logoUrl || ''}
                alt={store.storeName}
                className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover border-4 border-brand-500 shadow-2xl bg-white shrink-0"
              />
              <div className="space-y-1.5 text-white">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="px-2.5 py-0.5 bg-brand-500 text-ink-950 font-black text-[10px] rounded-md uppercase">
                    {store.category}
                  </span>
                  {store.verified && (
                    <span className="px-2.5 py-0.5 bg-brand-500/20 text-brand-400 border border-brand-500/40 font-bold text-[10px] rounded-md flex items-center gap-1">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              <a
                ...
                className="flex-1 sm:flex-initial px-5 py-2.5 bg-brand-600 hover:bg-brand-500 text-white font-black rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg transition"
              >
              ...
              <button
                ...
                className="p-2.5 bg-ink-800 hover:bg-ink-700 text-white font-bold rounded-xl text-xs border border-ink-700 transition cursor-pointer"
        ...
        {selectedProduct && (
          <div className="bg-gradient-to-br from-brand-500/10 via-ink-900 to-ink-900 border-2 border-brand-500/50 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <span className="px-3 py-1 bg-brand-500 text-ink-950 font-black text-xs rounded-full uppercase flex items-center gap-1.5">
                <ShoppingBag className="w-3.5 h-3.5" /> SẢN PHẨM ĐANG XEM
              </span>
              <button
                onClick={() => setSelectedProduct(null)}
                className="text-xs font-bold text-ink-400 hover:text-white"
              >
                Đóng xem chi tiết 
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="aspect-[4/3] rounded-2xl overflow-hidden bg-ink-950">
                <img loading="lazy"
              ...
              <div className="space-y-4 flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <div className="flex items-baseline gap-3">
                    ...
                    <span className="text-xs text-ink-400">/ {selectedProduct.unit}</span>
                  </div>
                  ...
                  <div className="p-3.5 bg-ink-800/80 rounded-xl text-xs text-ink-300 space-y-1">
                ...
                <div className="pt-4 flex items-center gap-3">
                  <button
                    ...
                    className="flex-1 py-3.5 bg-brand-500 hover:bg-brand-400 text-ink-950 font-black rounded-xl text-sm uppercase tracking-wide transition shadow-lg cursor-pointer"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {products.map(prod => (
              <div
                ...
                className="group bg-white dark:bg-ink-900 rounded-2xl overflow-hidden border border-ink-200 dark:border-ink-800 hover:border-brand-500 transition shadow-md hover:shadow-xl flex flex-col justify-between cursor-pointer"
              >
                <div className="aspect-[4/3] overflow-hidden bg-ink-950 relative">
                  <img loading="lazy"
                  ...
                  />
                  {prod.soldCount && (
                    <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-ink-950/80 text-brand-400 font-bold text-[10px] rounded-md backdrop-blur-sm">
                ...
                <div className="p-4 space-y-2 flex-1 flex flex-col justify-between">
                  <div className="space-y-1">
                    <h3 className="font-bold text-ink-900 dark:text-white text-sm group-hover:text-brand-500 transition line-clamp-2">
                      {prod.name}
                    </h3>
                    <p className="text-xs text-ink-500 line-clamp-2">{prod.description}</p>
                  </div>
                  ...
                  <div className="pt-2 flex items-baseline justify-between border-t border-ink-100 dark:border-ink-800">
                    ...
                    <span className="text-[11px] text-ink-400">/{prod.unit}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="bg-white dark:bg-ink-900 rounded-3xl p-6 sm:p-8 max-w-md w-full border border-ink-200 dark:border-ink-800 shadow-2xl space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-ink-400 hover:text-ink-600 dark:hover:text-white font-bold"
              >
            ...
            {orderSuccess ? (
              <div className="text-center py-6 space-y-3">
                ...
                <h4 className="text-base font-black text-ink-900 dark:text-white">ĐẶT HÀNG THÀNH CÔNG!</h4>
                <p className="text-xs text-ink-500 dark:text-ink-400">
                  Gian hàng <b>{store.storeName}</b> đã nhận được đơn hàng và sẽ liên hệ giao hàng trong 15-30 phút!
                </p>
                <button
                  onClick={() => setOrderModalOpen(false)}
                  className="px-6 py-2.5 bg-brand-600 text-white font-bold rounded-xl text-xs"
                >
                  Đóng
                </button>
              </div>
              ...
              <form onSubmit={handleOrderSubmit} className="space-y-4 text-xs">
                <div className="p-3 bg-ink-100 dark:bg-ink-800 rounded-xl flex items-center justify-between">
                  <div>
                    <div className="font-bold text-ink-900 dark:text-white">{selectedProduct.name}</div>
                    <div className="text-brand-500 font-bold">{selectedProduct.price.toLocaleString('vi-VN')}đ / {selectedProduct.unit}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => setOrderQty(Math.max(1, orderQty - 1))}
                      className="w-7 h-7 bg-ink-200 dark:bg-ink-700 rounded-lg font-bold"
                    >
                      -
                    </button>
                ...
                <div className="space-y-1">
                  <label className="font-bold text-ink-600 dark:text-ink-300">Họ và tên người nhận (*)</label>
                  <input
                    type="text"
                    required
                    value={customerName}
                    ...
                    className="w-full p-2.5 bg-ink-50 dark:bg-ink-800 border border-ink-300 dark:border-ink-700 rounded-xl font-bold"
                  />
                </div>

                <div className="space-y-1">
                  <label className="font-bold text-ink-600 dark:text-ink-300">Số điện thoại nhận hàng (*)</label>
                  <input
                    type="tel"
                    required
                    value={customerPhone}
                ...
                <div className="space-y-1">
                  <label className="font-bold text-ink-600 dark:text-ink-300">Địa chỉ giao hàng (Tòa/Phân khu/Số nhà)</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  <label className="font-bold text-ink-600 dark:text-ink-300">Ghi chú thêm</label>
                  <textarea
                    value={orderNote}
                ...
                <div className="p-3 bg-brand-500/10 border border-brand-500/30 rounded-xl flex items-center justify-between text-xs font-black">
                  <span>Tổng thanh toán dự kiến:</span>
                  <span className="text-brand-500 text-sm">{(selectedProduct.price * orderQty).toLocaleString('vi-VN')}đ</span>
                </div>

                <button
                  type="submit"
                  className="w-full py-3 bg-brand-500 hover:bg-brand-400 text-ink-950 font-black rounded-xl text-xs uppercase tracking-wider transition shadow-lg cursor-pointer"
                >
