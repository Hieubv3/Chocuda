    ...
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950 pb-20">
      ...
      <div className="bg-white dark:bg-ink-900 border-b border-ink-200 dark:border-ink-800 py-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center text-xs font-semibold text-ink-500 dark:text-ink-400 gap-1.5">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        {/* Header Hero */}
        <div className="bg-gradient-to-br from-ink-900 via-ink-900 to-brand-950 text-white rounded-3xl p-6 sm:p-10 border border-ink-800 shadow-xl space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-3 py-1 bg-brand-500 text-ink-950 font-black text-xs rounded-full uppercase flex items-center gap-1">
              <FileCode className="w-3.5 h-3.5" /> Chuẩn SEO Google & AI Index
            </span>
            <span className="px-3 py-1 bg-brand-500/20 text-brand-300 border border-brand-500/30 text-xs font-bold rounded-full">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-4 py-2 bg-brand-500 hover:bg-brand-400 text-ink-950 font-black text-xs rounded-xl flex items-center gap-1.5 transition shadow-md"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-4 py-2 bg-ink-800 hover:bg-ink-700 text-ink-200 font-bold text-xs rounded-xl flex items-center gap-1.5 transition"
            >
          ...
          {[
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer ${
                activeFilter === tab.id
                  ? 'bg-brand-500 text-ink-950 font-black shadow-xs'
                  : 'bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 text-ink-700 dark:text-ink-300 hover:border-brand-500'
              }`}
            >
              {tab.label}
            </button>
        ...
        {(activeFilter === 'all' || activeFilter === 'products') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              <h2 className="text-base font-black text-ink-900 dark:text-white flex items-center gap-2">
                <ShoppingBag className="w-4 h-4 text-brand-500" />
                <span>Bài Đăng Sản Phẩm & Hàng Hóa Cư Dân ({allProducts.length} Link Riêng)</span>
              </h2>
              <span className="text-[11px] font-mono text-ink-400">/gian-hang/:storeSlug/san-pham/:productId</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {allProducts.map(p => (
                <Link
                  key={p.id}
                  to={p.url}
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-brand-500/10 dark:hover:bg-brand-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl flex items-center gap-3 group transition"
                >
                  <img loading="lazy" src={p.images[0]} alt={p.name} className="w-12 h-12 rounded-xl object-cover shrink-0" />
                  <div className="min-w-0 flex-1">
                    <h3 className="font-black text-xs text-ink-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition truncate">
                      {p.name}
                    </h3>
        ...
        {(activeFilter === 'all' || activeFilter === 'stores') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/gian-hang/:storeSlug</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {INITIAL_USER_STOREFRONTS.map(store => (
                <Link
                  ...
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-brand-500/10 dark:hover:bg-brand-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl flex items-center gap-3 group transition"
                >
                  <img loading="lazy" src={store.logoUrl} alt={store.storeName} className="w-12 h-12 rounded-xl object-cover shrink-0 border border-brand-500" />
                  <div className="min-w-0 flex-1">
                    <h3 className="font-black text-xs text-ink-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition truncate">
                      {store.storeName}
                    </h3>
                    <span className="text-[11px] text-ink-500 dark:text-ink-400 block truncate">
        ...
        {(activeFilter === 'all' || activeFilter === 'services') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/dich-vu-cu-dan/:serviceSlug</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {INITIAL_RESIDENT_SERVICES.map(service => (
                <Link
                  ...
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-blue-500/10 dark:hover:bg-blue-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl flex items-center gap-3 group transition"
                >
        ...
        {(activeFilter === 'all' || activeFilter === 'jobs') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/tuyen-dung/viec-lam/:jobId</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {INITIAL_RECRUITMENT_JOBS.map(job => (
                <Link
                  ...
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-purple-500/10 dark:hover:bg-purple-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl space-y-1 group transition"
                >
        ...
        {(activeFilter === 'all' || activeFilter === 'cv') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/tuyen-dung/ung-vien/:candidateId</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {INITIAL_CANDIDATE_PROFILES.map(cand => (
                <Link
                  ...
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-teal-500/10 dark:hover:bg-teal-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl flex items-center gap-3 group transition"
                >
                  <img loading="lazy" 
                    src={cand.avatarUrl || ''} 
                    alt={cand.fullName} 
                  ...
                  />
                  <div className="min-w-0 flex-1">
                    <h3 className="font-black text-xs text-ink-900 dark:text-white group-hover:text-teal-600 dark:group-hover:text-teal-400 transition truncate">
                      {cand.fullName}
                    </h3>
                    <span className="text-[11px] text-brand-500 font-bold block truncate">
                      {cand.targetJobTitle}
                    </span>
                    <span className="text-[10px] text-ink-400 block truncate">
        ...
        {(activeFilter === 'all' || activeFilter === 'properties') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/:projectSlug/:propertyId</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {INITIAL_PROPERTIES.map(prop => (
                <Link
                  ...
                  className="p-3 bg-ink-50 dark:bg-ink-800/60 hover:bg-red-500/10 dark:hover:bg-red-500/10 border border-ink-200 dark:border-ink-700/80 rounded-2xl flex items-center gap-3 group transition"
                >
        ...
        {(activeFilter === 'all' || activeFilter === 'news') && (
          <section className="bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-ink-100 dark:border-ink-800 pb-3">
              ...
              <span className="text-[11px] font-mono text-ink-400">/tin-tuc/:category/:articleId</span>
            </div>
