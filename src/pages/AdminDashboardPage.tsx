...
{/* Khu vực Quản Trị Tuyển Dụng & Phí Mở Khóa Hồ Sơ */}
<div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-6 mt-6">
  <div>
    <h3 className="text-lg font-extrabold text-slate-900 dark:text-slate-100">⚙️ Quản Trị Hệ Thống Tuyển Dụng & Việc Làm</h3>
    <p className="text-xs text-slate-500 mt-1">Cấu hình mức phí mở khóa hồ sơ ứng viên và kiểm duyệt CV nộp vào hệ thống.</p>
  </div>
  
  {/* Cấu hình số tiền thu phí */}
  <div className="bg-emerald-50 dark:bg-emerald-950/40 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
    <div className="w-full md:w-2/3">
      <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Mức phí mở khóa xem liên hệ / tải CV ứng viên (VNĐ)</label>
      <input 
        type="number" 
        defaultValue={50000}
        className="w-full px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-bold text-emerald-600" 
      />
    </div>
    <button 
      onClick={() => alert('Đã lưu cấu hình mức phí mở khóa hồ sơ thành công!')}
      className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm shadow transition mt-2 md:mt-5"
    >
      Lưu Mức Phí
    </button>
  </div>

  {/* Quản lý danh sách ứng viên đã đăng */}
  <div>
    <h4 className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-3">Danh Sách Hồ Sơ Ứng Viên Chờ Kiểm Duyệt</h4>
    <div className="overflow-x-auto">
      <table className="w-full text-left text-xs">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-500">
            <th className="pb-3 font-bold">Họ Tên</th>
            <th className="pb-3 font-bold">Vị Trí Mong Muốn</th>
            <th className="pb-3 font-bold">SĐT / Zalo</th>
            <th className="pb-3 font-bold">Lương Kỳ Vọng</th>
            <th className="pb-3 font-bold text-right">Thao Tác</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          <tr>
            <td className="py-3 font-bold text-slate-900 dark:text-slate-100">Nguyễn Văn A</td>
            <td className="py-3 text-slate-600 dark:text-slate-400">Thợ Điện Nước Tổng Hợp</td>
            <td className="py-3 text-emerald-600 font-bold">0912345xxx</td>
            <td className="py-3 text-slate-600 dark:text-slate-400">8 - 10 triệu</td>
            <td className="py-3 text-right space-x-2">
              <button onClick={() => alert('Đã duyệt hồ sơ hiển thị công khai.')} className="px-2.5 py-1 bg-emerald-100 text-emerald-700 font-bold rounded-lg hover:bg-emerald-200">Duyệt</button>
              <button onClick={() => alert('Đã xóa hồ sơ ứng viên khỏi hệ thống.')} className="px-2.5 py-1 bg-red-100 text-red-700 font-bold rounded-lg hover:bg-red-200">Xóa</button>
            </td>
          </tr>
        </tbody>
      </table>
