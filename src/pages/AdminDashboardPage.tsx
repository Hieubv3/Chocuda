...
export const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({
  ...
  const [projectTreeOpen, setProjectTreeOpen] = useState(false); // dropdown lọc dự án (mở/đóng)
  const projectTreeRef = useRef<HTMLDivElement>(null); // ref để đóng dropdown khi click ra ngoài
  ...
  // Đóng dropdown lọc dự án khi click ra ngoài
  useEffect(() => {
    if (!projectTreeOpen) return;
    const handleClickOutside = (e: MouseEvent) => {
      if (projectTreeRef.current && !projectTreeRef.current.contains(e.target as Node)) {
        setProjectTreeOpen(false);
      }
    };
      ...
      {activeTab === 'projects' && (
        <div className="bg-white dark:bg-slate-800/90 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 shadow-xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch">
            ...
            <div className="relative" ref={projectTreeRef}>
              <button
                type="button"
                onClick={() => setProjectTreeOpen(prev => !prev)}
              ...
              >
                ...
                <ChevronDown className={`w-3 h-3 ml-auto transition-transform ${projectTreeOpen ? 'rotate-180' : ''}`} />
              </button>
              {/* Tree dropdown — chỉ hiển thị khi bấm mở */}
              {projectTreeOpen && (
              <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-20 max-h-80 overflow-y-auto">
                          ...
                          {children.map((child) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() => { setProjectCategoryFilter(child.id); setProjectTreeOpen(false); }}
                              >
                ...
                {projects.filter(p => !p.parentId && !parentProjects.includes(p)).map((p) => (
                  <div
                    ...
                    onClick={() => { setProjectCategoryFilter(p.id); setProjectTreeOpen(false); }}
                  >
