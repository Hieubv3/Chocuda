  ...
  const handleUnlockCandidate = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        alert(` ${data.error || 'Không thể mở khóa hồ sơ!'}`);
      }
    } catch (err) {
      alert(' Lỗi kết nối máy chủ khi mở khóa hồ sơ!');
    } finally {
  ...
  if (!candidate) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-4">
        <h1 className="text-2xl font-black text-ink-900 dark:text-white">Không tìm thấy hồ sơ ứng viên</h1>
        <p className="text-xs text-ink-500">Hồ sơ ứng viên có thể đã ẩn hoặc tìm được việc làm phù hợp.</p>
      ...
      <div className="bg-white dark:bg-ink-900 border-b border-ink-200 dark:border-ink-800 py-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center text-xs font-semibold text-ink-500 dark:text-ink-400 overflow-x-auto whitespace-nowrap gap-1.5">
            ...
            <ChevronRight className="w-3.5 h-3.5 shrink-0 text-ink-400" />
            <span className="text-ink-600 dark:text-ink-300 font-medium">Hồ sơ ứng viên</span>
            <ChevronRight className="w-3.5 h-3.5 shrink-0 text-ink-400" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex items-start gap-4">
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-3xl overflow-hidden border-2 border-brand-500 shadow-md shrink-0 bg-ink-100 dark:bg-ink-800">
                <img loading="lazy"
                  src={candidate.avatar || ''}
                  alt={candidate.fullName}
                  className="w-full h-full object-cover"
                />
                <div className="absolute bottom-0 inset-x-0 bg-brand-600 text-white text-[9px] font-black text-center py-0.5 uppercase">
                  Cư Dân
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="px-2.5 py-0.5 bg-brand-500/10 text-brand-600 dark:text-brand-400 text-[11px] font-black rounded-full border border-brand-500/20 flex items-center gap-1">
                ...
                <p className="text-sm font-extrabold text-brand-600 dark:text-brand-400 flex items-center gap-1.5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm text-xs">
              ...
              <div className="space-y-1">
                ...
                <span className="text-sm font-bold text-ink-800 dark:text-ink-200 block truncate">
                  {project?.name || candidate.desiredProject}
                </span>
              </div>
            </div>
            ...
            {/* Bio & Intro Summary */}
            <div className="p-6 sm:p-8 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm space-y-4">
              ...
              <p className="text-xs sm:text-sm text-ink-700 dark:text-ink-300 leading-relaxed whitespace-pre-line">
              ...
              <div className="p-6 sm:p-8 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {candidate.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-3.5 py-1.5 bg-ink-100 dark:bg-ink-800 text-ink-800 dark:text-ink-200 text-xs font-bold rounded-xl border border-ink-200 dark:border-ink-700 flex items-center gap-1.5"
                    >
            ...
            {candidate.workExperience && candidate.workExperience.length > 0 && (
              <div className="p-6 sm:p-8 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm space-y-5">
                ...
                <div className="space-y-4">
                  {candidate.workExperience.map((exp, idx) => (
                    <div key={idx} className="p-4 bg-ink-50 dark:bg-ink-800/60 rounded-2xl border border-ink-100 dark:border-ink-700 space-y-2">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="text-[11px] bg-ink-200 dark:bg-ink-700 text-ink-700 dark:text-ink-300 font-bold px-2 py-0.5 rounded-lg">
                          {exp.duration}
                        </span>
                      </div>
            ...
            {candidate.education && candidate.education.length > 0 && (
              <div className="p-6 sm:p-8 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm space-y-5">
                <h3 className="text-base font-black text-ink-900 dark:text-white uppercase tracking-wider text-purple-500 flex items-center gap-2">
                ...
                <div className="space-y-3">
                  {candidate.education.map((edu, idx) => (
                    <div key={idx} className="p-4 bg-ink-50 dark:bg-ink-800/60 rounded-2xl border border-ink-100 dark:border-ink-700 flex items-start justify-between text-xs">
                      <div>
                        <h4 className="font-bold text-sm text-ink-900 dark:text-white">{edu.major}</h4>
                        <p className="text-ink-500 dark:text-ink-400">{edu.school}</p>
                      </div>
                      <span className="text-ink-500 font-bold">{edu.year}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div id="invite-section" className="p-6 sm:p-8 bg-gradient-to-br from-brand-950/40 via-ink-900 to-ink-900 rounded-3xl border border-brand-500/30 shadow-xl space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-white dark:bg-ink-900 rounded-3xl border border-ink-200 dark:border-ink-800 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-md ${
                  isUnlocked ? 'bg-brand-500/10 text-brand-600 dark:text-brand-400' : 'bg-brand-500/10 text-brand-600 dark:text-brand-400'
                }`}>
              ...
              {isUnlocked ? (
                <div className="space-y-2.5 text-xs">
                  <div className="p-3 bg-brand-50 dark:bg-brand-950/40 rounded-2xl border border-brand-200 dark:border-brand-800 space-y-1">
                    <div className="text-[11px] text-ink-500 dark:text-ink-400 font-medium">Số điện thoại liên hệ:</div>
                    <div className="text-base font-black text-brand-700 dark:text-brand-300 font-mono tracking-wider">
                      {candidate.phone}
                    </div>
                    {candidate.email && (
                      <div className="text-[11px] text-ink-600 dark:text-ink-300 font-medium pt-1 border-t border-brand-200/60 dark:border-brand-800/60">
                  ...
                  <a
                    ...
                    className="w-full py-2.5 px-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow transition"
                  >
                  ...
                  {candidate.attachedCvUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-2.5 px-3 bg-ink-800 hover:bg-ink-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow transition text-[11px]"
                    >
                ...
                <div className="space-y-3 text-xs">
                  <div className="p-3.5 bg-ink-50 dark:bg-ink-800/80 rounded-2xl border border-dashed border-ink-300 dark:border-ink-700 space-y-2">
                    <div className="flex items-center justify-between text-ink-500 dark:text-ink-400 font-mono">
                      <span>SĐT:</span>
                  ...
                  <p className="text-[11px] text-ink-500 dark:text-ink-400 leading-relaxed">
                     Để bảo mật thông tin cá nhân của cư dân, chỉ tài khoản <b>Nhà Tuyển Dụng</b> đã đăng ký hoặc sử dụng <b>Token Cư Dân</b> mới có quyền mở khóa xem đầy đủ số điện thoại và liên hệ trực tiếp.
                  </p>

                  <button
                    onClick={handleUnlockCandidate}
                    disabled={isUnlocking}
                    className="w-full py-3 bg-brand-500 hover:bg-brand-400 active:bg-brand-600 text-ink-950 font-black rounded-xl flex items-center justify-center gap-2 shadow-lg transition cursor-pointer"
                  >
                  ...
                  {!currentUser && (
                    <button
                      onClick={onOpenAuth}
                      className="w-full py-2 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 text-ink-700 dark:text-ink-300 font-bold rounded-xl text-[11px] transition text-center"
                    >
            ...
            {/* Post Job CTA */}
            <div className="p-6 bg-gradient-to-br from-ink-900 to-ink-950 text-white rounded-3xl border border-ink-800 shadow-xl space-y-3">
              ...
              <Link
                to="/tuyen-dung"
                className="block w-full text-center py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow transition"
              >
                  ...
                  {relatedCandidates.map(rc => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-brand-600 dark:text-brand-400 font-bold truncate max-w-[140px]">{rc.desiredPosition}</span>
                        <span className="text-ink-500">{rc.experienceDisplay || 'Kinh nghiệm'}</span>
                      </div>
                    </Link>
                  ))}
                </div>
