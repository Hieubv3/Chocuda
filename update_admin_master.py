...
# 1. Expand activeTab type to include 'all_products_mgmt'
old_tab_type = "| 'resident_services_mgmt' | 'recruitment_mgmt' | 'stores_mgmt' | 'orders_mgmt' | 'partners_reputation' | 'resident_finance' | 'package_orders_mgmt'"
new_tab_type = "| 'resident_services_mgmt' | 'recruitment_mgmt' | 'stores_mgmt' | 'all_products_mgmt' | 'orders_mgmt' | 'partners_reputation' | 'resident_finance' | 'package_orders_mgmt'"

if old_tab_type in text:
    text = text.replace(old_tab_type, new_tab_type)
    print("1. Updated activeTab union type")

# 2. Update effectiveMainTab check
old_eff = "if (['stores_mgmt', 'orders_mgmt', 'package_orders_mgmt', 'resident_finance', 'partners_reputation'].includes(activeTab)) return 'resident_market';"
new_eff = "if (['stores_mgmt', 'all_products_mgmt', 'orders_mgmt', 'package_orders_mgmt', 'resident_finance', 'partners_reputation'].includes(activeTab)) return 'resident_market';"

if old_eff in text:
    text = text.replace(old_eff, new_eff)
    print("2. Updated effectiveMainTab condition")

# 3. Add 'Tất Cả Sản Phẩm & Món Ăn' to sidebar menu under Chợ Cư Dân
old_sidebar_items = """                  <button
...
if old_sidebar_items in text:
    text = text.replace(old_sidebar_items, new_sidebar_items)
    print("3. Updated sidebar menu items")

# 4. Add product quick moderation / batch handlers and dedicated all_products_mgmt tab view
all_products_code = """
...
# Insert all_products_code right before stores_mgmt view
stores_view_marker = "{/* ==================== TAB GIAN HÀNG & DỊCH VỤ CƯ DÂN ==================== */}"
if stores_view_marker in text:
    text = text.replace(stores_view_marker, all_products_code + "\n      " + stores_view_marker)
    print("4. Added all_products_mgmt view before stores_mgmt view")

with open('src/pages/AdminDashboardPage.tsx', 'w', encoding='utf-8') as f:
    f.write(text)

print("Finished update_admin_master.py successfully")
